...
import type { CommandType, PresentationEventPayload, SeatGridView, SeatView, SlotView, StateSnapshotView } from "@cambio/protocol";
import {
  Button,
  CardGrid,
  Dialog,
...
export function GameTable({
  ...
  const seats = useMemo(() => [...snapshot.seats].sort((left, right) => left.seatIndex - right.seatIndex), [snapshot.seats]);
  const names = useMemo(() => new Map(snapshot.seats.map((seat) => [seat.playerId, seat.displayName])), [snapshot.seats]);
  const viewer = seats.find((seat) => seat.playerId === snapshot.viewerSeatId);
  const activeName = snapshot.round.activePlayerId === null ? "No active player" : names.get(snapshot.round.activePlayerId) ?? snapshot.round.activePlayerId;
  const countdown = useSnapCountdown(snapshot.snapWindow);
  const transientReveals = presentationEvents.flatMap((event) => event.type === "wrongSnapReveal" ? [{ ...event.target, card: event.card }] : []);
  ...
  const drawnOwnerName = snapshot.drawnCard.state === "none" ? undefined : names.get(snapshot.drawnCard.playerId);
  ...
  function slotAction(grid: SeatGridView, slot: SlotView): SlotAction | null {
    ...
    if (mode === "power" && powerCanTarget(snapshot, grid.playerId, slot)) {
      const selected = snapshot.pendingPower?.selections.some((target) => target.playerId === grid.playerId && target.slotId === slot.slotId) ?? false;
      const command = legal.has("reselectPowerTarget") ? "reselectPowerTarget" : "selectPowerTarget";
      return {
        label: legal.has("reselectPowerTarget") ? "Reselect power target" : "Select power target",
        selected,
        onSelect: () => send(command, command === "reselectPowerTarget"
  ...
  return (
    <div className="game-table">
      ...
      <LiveRegion>{countdown === null ? "" : `Snap window ${Math.ceil(countdown / 1_000)} seconds remaining.`}</LiveRegion>
      <LiveRegion politeness="assertive">{lastError ?? pauseAnnouncement(snapshot, names) ?? transientAnnouncement(presentationEvents)}</LiveRegion>
      ...
      <header className="room-header panel">
        <div>
          ...
          <p>
            Round {snapshot.round.roundNumber ?? "—"} · {phaseLabel(snapshot)} · Active: {activeName}
          </p>
          {snapshot.round.cambio === null ? null : (
            <p className="final-turn-banner">
              Cambio called by {names.get(snapshot.round.cambio.callerId) ?? snapshot.round.cambio.callerId}. Final turns: {snapshot.round.cambio.finalTurnQueue.map((playerId) => names.get(playerId) ?? playerId).join(", ")}.
            </p>
          )}
        </div>
      ...
      <section className="table-surface panel" aria-label="Authoritative card table">
        <div className="table-center">
          <PileSummary piles={snapshot.piles} />
          <DrawnCardTray
          ...
          {snapshot.snapWindow === null ? null : (
            <div className="snap-countdown" role="timer" aria-label="Snap countdown">
              ...
              <progress value={(countdown ?? snapshot.snapWindow.remainingMs)} max={snapshot.snapWindow.durationMs} />
            </div>
          )}
        </div>
      ...
      <section className="action-bar panel" aria-labelledby="actions-title">
        ...
        <div className="action-bar__buttons">
          <Button variant="primary" disabled={!legal.has("acknowledgeOpeningPeek")} onClick={() => send("acknowledgeOpeningPeek")}>
            Acknowledge opening peek
          </Button>
          <Button variant="primary" disabled={!legal.has("drawCard")} onClick={() => send("drawCard")}>Draw card</Button>
          ...
          <Button disabled={!legal.has("skipPower") || !pendingPowerOwned} onClick={() => send("skipPower")}>Skip power</Button>
          <Button
          ...
          <Button variant="primary" disabled={!legal.has("readyForNextRound")} onClick={() => send("readyForNextRound")}>Ready for next round</Button>
      ...
      <div className="side-panels">
        <Scoreboard seats={snapshot.seats} scores={snapshot.scores} />
        <section className="ui-card" aria-labelledby="lifecycle-title">
          <div className="ui-card__header">
            <h2 id="lifecycle-title">Host and lifecycle</h2>
          </div>
          <p>Host: {snapshot.room.hostPlayerId === null ? "none" : names.get(snapshot.room.hostPlayerId) ?? snapshot.room.hostPlayerId}</p>
          {seats.filter((seat) => seat.removalEligible).map((seat) => (
            <Button
              key={seat.playerId}
          ...
          <Button variant="danger" disabled={viewer?.isHost !== true || snapshot.room.status === "complete"} onClick={() => setHostEndOpen(true)}>
            End match
          </Button>
        </section>
      ...
      {snapshot.room.status === "intermission" ? (
        <section className="panel result-panel" aria-labelledby="round-results-title">
          ...
          <p>Ready players: {snapshot.seats.filter((seat) => seat.readyForNextRound).length}/{snapshot.seats.filter((seat) => seat.connection !== "removed").length}</p>
        </section>
      ) : null}
      ...
      {snapshot.room.status === "complete" ? (
        <section className="panel result-panel" aria-labelledby="match-summary-title">
          ...
          <p>Winner{winners.length === 1 ? "" : "s"}: {winners.map((playerId) => names.get(playerId) ?? playerId).join(", ")}.</p>
        </section>
      ) : null}

      <Dialog title="Private power reveal" open={pendingPowerOwned && snapshot.pendingPower?.stage === "awaitingRevealAck"} onClose={() => send("acknowledgePowerReveal")}>
        <p>Only your server-authorized view is shown here.</p>
        <ul>
          {privateRevealSlots.map(({ seat, slot }) => (
            <li key={`${seat.playerId}-${slot.slotId}`}>{seat.displayName}, {slotPositionLabel(slot)}: {slot.state === "revealed" ? formatCard(slot.card) : "face down"}</li>
          ))}
        </ul>
        <Button variant="primary" disabled={!legal.has("acknowledgePowerReveal")} onClick={() => send("acknowledgePowerReveal")}>Acknowledge reveal</Button>
      </Dialog>

      <Dialog title="Black King decision" open={pendingPowerOwned && snapshot.pendingPower?.stage === "awaitingKingDecision"} onClose={() => send("decideBlackKingSwap", { decision: "decline" })}>
        <p>Confirm to swap the selected positions, or decline to leave them in place.</p>
        <div className="action-bar__buttons">
          <Button variant="primary" disabled={!legal.has("decideBlackKingSwap")} onClick={() => send("decideBlackKingSwap", { decision: "confirm" })}>Confirm swap</Button>
...
function phaseLabel(snapshot: StateSnapshotView): string {
  ...
  return snapshot.round.turnStage === null ? snapshot.round.phase : `${snapshot.round.phase} / ${snapshot.round.turnStage}`;
}

function pauseAnnouncement(snapshot: StateSnapshotView, names: ReadonlyMap<string, string>): string | null {
...
function transientAnnouncement(events: readonly PresentationEventPayload[]): string {
  return events.some((event) => event.type === "wrongSnapReveal") ? "Wrong snap reveal shown briefly." : "";
}

function privateRevealTargets(snapshot: StateSnapshotView): { readonly seat: SeatView; readonly slot: SlotView }[] {
...
function matchWinners(snapshot: StateSnapshotView): string[] {
  const completed = [...snapshot.actionLog].reverse().find((entry) => entry.type === "matchCompleted");
  ...
  return snapshot.scores.filter((score) => score.cumulativeScore === lowest).map((score) => score.playerId);
}

function callerAdjustmentText(snapshot: StateSnapshotView, names: ReadonlyMap<string, string>): string {
  ...
  const callerScore = snapshot.scores.find((score) => score.playerId === snapshot.round.cambio?.callerId);
